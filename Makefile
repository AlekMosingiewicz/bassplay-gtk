...
pkgdatadir = $(datadir)/bassplay_gtk
pkgincludedir = $(includedir)/bassplay_gtk
pkglibdir = $(libdir)/bassplay_gtk
pkglibexecdir = $(libexecdir)/bassplay_gtk
...
am__base_list = \
  sed '$$!N;$$!N;$$!N;$$!N;$$!N;$$!N;$$!N;s/\n/ /g' | \
  sed '$$!N;$$!N;$$!N;$$!N;s/\n/ /g'
am__installdirs = "$(DESTDIR)$(bassplay_gtkdocdir)"
DATA = $(bassplay_gtkdoc_DATA)
...
LDFLAGS = 
LIBOBJS = 
LIBS = -lpthread -lgthread-2.0 -lbass 
...
OTOOL = 
OTOOL64 = 
PACKAGE = bassplay_gtk
PACKAGE_BUGREPORT = 
PACKAGE_NAME = bassplay_gtk
PACKAGE_STRING = bassplay_gtk 0.1.6
PACKAGE_TARNAME = bassplay_gtk
PACKAGE_URL = 
...
install-bassplay_gtkdocDATA: $(bassplay_gtkdoc_DATA)
	@$(NORMAL_INSTALL)
	test -z "$(bassplay_gtkdocdir)" || $(MKDIR_P) "$(DESTDIR)$(bassplay_gtkdocdir)"
	@list='$(bassplay_gtkdoc_DATA)'; test -n "$(bassplay_gtkdocdir)" || list=; \
	for p in $$list; do \
	  if test -f "$$p"; then d=; else d="$(srcdir)/"; fi; \
	  echo "$$d$$p"; \
	done | $(am__base_list) | \
	while read files; do \
	  echo " $(INSTALL_DATA) $$files '$(DESTDIR)$(bassplay_gtkdocdir)'"; \
	  $(INSTALL_DATA) $$files "$(DESTDIR)$(bassplay_gtkdocdir)" || exit $$?; \
	done

uninstall-bassplay_gtkdocDATA:
	@$(NORMAL_UNINSTALL)
	@list='$(bassplay_gtkdoc_DATA)'; test -n "$(bassplay_gtkdocdir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	test -n "$$files" || exit 0; \
	echo " ( cd '$(DESTDIR)$(bassplay_gtkdocdir)' && rm -f" $$files ")"; \
	cd "$(DESTDIR)$(bassplay_gtkdocdir)" && rm -f $$files
...
installdirs-am:
	for dir in "$(DESTDIR)$(bassplay_gtkdocdir)"; do \
...
install-data-am: install-bassplay_gtkdocDATA
...
uninstall-am: uninstall-bassplay_gtkdocDATA uninstall-local
...
.PHONY: $(RECURSIVE_CLEAN_TARGETS) $(RECURSIVE_TARGETS) CTAGS GTAGS \
	...
	install install-am install-bassplay_gtkdocDATA install-data \
	...
	uninstall uninstall-am uninstall-bassplay_gtkdocDATA \
	uninstall-local
